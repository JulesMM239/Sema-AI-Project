...
def run(job_id: int, model_version: str):
    """Prepare an ASR dataset manifest from uploaded WAV files.

    This job validates audio files and writes `manifest.jsonl` with:
      {"path": "...", "sr": 16000, "seconds": 3.2, "dialect": "KIV", "text": "..."}

    Admins can then use this manifest to fine-tune an ASR model offline.
    """

    update_job(job_id, "running", notes="Preparing ASR dataset manifest from uploaded audio")
    ...
    out_dir = out_root / "asr" / model_version
    ...
    update_job(job_id, "done", notes=f"ASR manifest ready: {kept} files kept, {dropped} dropped.", model_version=model_version)
